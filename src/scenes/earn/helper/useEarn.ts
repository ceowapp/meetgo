...
const useEarn = (propsEarn?: IDataEarn) => {
  ...
  const startEarn = async (data: IReqCurrentShop) => {
    try {
      ...
      setErrorMessage(
        t('earning.locationError', {
          warning: t('earning.warningTitle'),
          moveTip: t('earning.moveTip'),
        })
      );
      const errorMess = error as IResponseType<IStatus>;
      addToast({
        message: errorMess?.status?.message || t('earning.startError'),
        position: 'top',
        type: 'ERROR_V3',
      });
    } finally {
  ...
  const checkEarn = async (data: IReqCurrentEarn) => {
    try {
      ...
      const errorMess = error as IResponseType<IStatus>;
      addToast({
        message: errorMess?.status?.message || t('earning.checkError'),
        position: 'top',
        type: 'ERROR_V3',
      });
      return false;
  ...
  const verifyEarn = async (data: IReqCurrentEarn): Promise<boolean> => {
    try {
      ...
      const errorMess = error as IResponseType<IStatus>;
      addToast({
        message: errorMess?.status?.message || t('earning.verifyError'),
        position: 'top',
        type: 'ERROR_V3',
      });
      setLoading(false);
  ...
  const cancelEarn = async (data: IReqCurrentEarn) => {
    try {
      ...
      const errorMess = error as IResponseType<IStatus>;
      addToast({
        message: errorMess?.status?.message || t('earning.cancelError'),
        position: 'top',
        type: 'ERROR_V3',
      });
    } finally {
  ...
  const historyEarn = async () => {
    try {
      ...
      const errorMess = error as IResponseType<IStatus>;
      addToast({
        message: errorMess?.status?.message || t('earning.historyError'),
        position: 'top',
        type: 'ERROR_V3',
      });
    } finally {
