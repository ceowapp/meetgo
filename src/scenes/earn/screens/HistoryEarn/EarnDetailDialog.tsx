  ...
  return (
    <View style={styles.container}>
      <View style={styles.rowContent}>
        <Text style={styles.label}>{t('earning.location')}:</Text>
        <Text style={styles.content}>{dataEarn.address}</Text>
      </View>
      <View style={styles.rowContent}>
        <Text style={styles.label}>{t('earning.time')}:</Text>
        <Text style={styles.content}>{dataEarn.createdAt}</Text>
      </View>
      <View style={styles.rowContent}>
        <Text style={styles.label}>{t('earning.status')}:</Text>
        <Text style={[styles.content, {color}]}>{dataEarn.statusMessage}</Text>
      </View>
      <View style={styles.rowContent}>
        <Text style={styles.label}>{t('earning.points')}:</Text>
        <Text style={[styles.content, {color}]}>
          {dataEarn.meetpointForAccount || 0}
        </Text>
      </View>
      <ButtonPrimary content={t('earning.close')} onPress={() => hideModal()} />
    </View>
  );
};
